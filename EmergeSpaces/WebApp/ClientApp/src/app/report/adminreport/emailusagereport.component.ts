...
@Component({
  selector: 'email-usage-report',
  templateUrl: './emailusagereport.component.html'
})
export class EmailUsageReportComponent implements OnInit {
  ...
  toggleAllSelection() {
   ...
   downloadPdf() {
    ...
    doc.save('email_usage_report' + '.pdf');
  }
  ...
  ExportTOExcel()
  {
    let data  = this.dataSource.data.map((x:any) =>({
      ...
      'Email Campaign':x.campaignCount,
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Email Usage Report');

    /* save to file */
    XLSX.writeFile(wb, 'email_usage_report.xlsx');
   ...
   onGenerate(){
    ...
    this.adminreport.generateEmailUsageReport(this.formGroup.value).subscribe(
      res=>
      {
