...
export class LeadGridComponent implements OnInit {
  ...
  public leadList: any;selectionAmount;
  ...
  owners;sources;stages;

  leadStageFilter;leadOwnerFilter;leadSourceFilter;

  dateTypeFilter;
  form;

  constructor(private _service: LeadService,private router: Router,private dialog:MatDialog,
    private sharedProjectService:SharedProjectService,private formBuilder: FormBuilder,
  ...
  ngOnInit() {
    ...
    this.getLeadSources();
      ...
      this.sharedProjectService.data.subscribe(data => {
        if(data && this.router.url == '/lead/lead-list'){
  ...
  getLeadOwners(){
    this._service.getPresaleLeadOwners(this.selectedProject).subscribe(res => {
      this.owners = res;
    });
  }
  ...
  showBulkUpdate() {
    ...
    const ref = this.dialog.open(BulkUpdateComponent,dialogConfig);

    ref.afterClosed().subscribe(result => {
      if(result != null){
        this._service.bulkUpdate(result).subscribe(res => {
          this.messageService.success('Lead updated successfully.');
          this.getAllLeads();
        },
        error => {
